...
const Sidebar = () => {
  ...
  const { data: userData } = useSWR(_apiBase + `/me`, async (url) => {
  ...
  return (
    <div className="h-screen w-56 bg-slate-100 flex flex-col">
      ...
      <ul className="p-[10px] flex flex-col gap-4">
        <li
          className="cursor-pointer pr-1 relative flex justify-between items-center text-slate-800 font-medium text-xl transition group"
          onClick={() => {
            router.push("/goods");
      ...
      <div className="mt-auto p-5 text-slate-900">
        <div className="flex space-x-2 mb-4">
          {userData?.data.roles.map((role: any) => (
            <span key={role.id} className="bg-gray-300 px-2 py-1 rounded text-sm">
              {role.name}
            </span>
          ))}
        </div>
